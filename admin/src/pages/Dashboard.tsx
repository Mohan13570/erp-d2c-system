...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto pb-12 print:bg-white print:p-0">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 print:hidden">
        <div>
          <div className="text-sm text-slate-500 mb-1 flex items-center space-x-2">
            <span>Home</span>
          ...
          <p className="text-slate-500 font-medium mt-1">Real-time overview of your enterprise operations and D2C sales.</p>
        </div>
        <div className="flex gap-3">
          <button onClick={toggleTimeRange} className="bg-white border border-slate-200 text-slate-700 px-4 py-2 rounded-lg text-sm font-semibold shadow-sm hover:bg-slate-50 flex items-center transition-colors">
            <Clock size={16} className="mr-2 text-slate-500"/> {timeRange}
          </button>
          <button onClick={handlePrintReport} className="bg-blue-600 text-white px-5 py-2 rounded-lg text-sm font-bold shadow-lg shadow-blue-500/30 hover:bg-blue-700 transition-colors">
            Generate Report
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow">
          <div className="absolute top-0 right-0 p-5 opacity-10 group-hover:opacity-20 transition-opacity"><DollarSign size={80} className="text-emerald-500 translate-x-4 -translate-y-4"/></div>
          <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-2"><DollarSign size={14} className="text-blue-600"/> Net Revenue</p>
          <p className="text-2xl font-black text-slate-900 mb-2">${data.kpis.revenue.toLocaleString()}</p>
          <div className="flex items-center text-[10px] font-bold text-emerald-600">
            {data.kpis.revenue > 0 ? <><TrendingUp size={14} className="mr-1"/> <span>Active</span></> : <span className="text-slate-400">No data yet</span>}
          </div>
        </div>

        <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow">
          <div className="absolute top-0 right-0 p-5 opacity-10 group-hover:opacity-20 transition-opacity"><ShoppingCart size={80} className="text-blue-500 translate-x-4 -translate-y-4"/></div>
          <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-2"><ShoppingCart size={14} className="text-blue-600"/> Total Orders</p>
          <p className="text-2xl font-black text-slate-900 mb-2">{data.kpis.totalSales}</p>
          <div className="flex items-center text-[10px] font-bold text-emerald-600">
             {data.kpis.totalSales > 0 ? <><TrendingUp size={14} className="mr-1"/> <span>Active</span></> : <span className="text-slate-400">No data yet</span>}
          </div>
        </div>

        <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow">
          <div className="absolute top-0 right-0 p-5 opacity-10 group-hover:opacity-20 transition-opacity"><Truck size={80} className="text-amber-500 translate-x-4 -translate-y-4"/></div>
          <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-2"><Truck size={14} className="text-blue-600"/> Active Shipments</p>
          <p className="text-2xl font-black text-slate-900 mb-2">{data.kpis.activeShipments}</p>
          <div className="flex items-center text-[10px] font-bold text-amber-500">
            <span className="text-slate-400">Current active</span>
          </div>
        </div>

        <div className="bg-[#0F172A] p-5 rounded-xl border border-slate-800 shadow-lg relative overflow-hidden text-white group hover:shadow-xl transition-shadow">
          <div className="absolute top-0 right-0 p-5 opacity-10 group-hover:opacity-20 transition-opacity"><Users size={80} className="translate-x-4 -translate-y-4"/></div>
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-2"><Users size={14} className="text-blue-500"/> System Users</p>
          <p className="text-2xl font-black text-white mb-2">{data.kpis.totalUsers}</p>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm lg:col-span-2">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2"><div className="w-3 h-3 rounded-full bg-blue-500"></div><span className="text-xs font-semibold text-slate-600">Revenue</span></div>
              <div className="flex items-center space-x-2"><div className="w-3 h-3 rounded-full bg-slate-200"></div><span className="text-xs font-semibold text-slate-600">Expenses</span></div>
            </div>
          </div>
          <div className="h-[300px] w-full">
            {data.revenueForecast?.length === 0 || data.revenueForecast?.every((i: any) => i.revenue === 0 && i.expenses === 0) ? (
               <div className="h-full flex items-center justify-center border-2 border-dashed border-slate-100 rounded-2xl">
                 <p className="text-slate-400 font-bold">No financial data available to chart.</p>
               </div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.revenueForecast} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  <defs>
                  ...
                  <Area type="monotone" dataKey="expenses" stroke="#cbd5e1" strokeWidth={3} fillOpacity={0.2} fill="#e2e8f0" />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)]">
          ...
          <div className="h-[300px]">
             {!data.logisticsData || data.logisticsData.length === 0 || data.logisticsData.every((i: any) => i.shipped === 0 && i.delivered === 0) ? (
               <div className="h-full flex items-center justify-center border-2 border-dashed border-gray-100 rounded-2xl">
